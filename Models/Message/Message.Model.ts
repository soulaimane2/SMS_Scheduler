...
export async function updateSMSStatus(
  ...
  try {
    const sms = await messageModel
      .findByIdAndUpdate(id, { status: status, DeliveryTime: deliveryTime })
      .exec();
...
export async function getSMSByScheduleId({
  schedule_id,
  skip,
  PER_PAGE,
  status,
  ...
  try {
    ...
    if (schedule !== undefined) {
      const sms = await messageModel
        .find(
          { dnis: schedule?.contact, ...opts, [stat]: status },
          { _id: 0, message_id: 0, __v: 0 },
          { skip, limit: PER_PAGE }
        )
        .exec();
